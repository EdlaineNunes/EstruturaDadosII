...
void ordendarPorSelecao(Cliente *dados, int n)
{ 
	Cliente temporario;
	int indiceMenorCodigo;
	
	for(int i = 0; i < n - 1; i++) { 
		indiceMenorCodigo = i;
		for(int j = i + 1; j < n; j++) 
			if (dados[j].codigo < dados[indiceMenorCodigo].codigo) 
				indiceMenorCodigo = j;
		temporario = dados[indiceMenorCodigo]; 
		dados[indiceMenorCodigo] = dados[i]; 
		dados[i] = temporario;
	}
}
...
int main(int argc, char *argv[])
{
	...
	ordendarPorSelecao(dados, TAMANHO_MAXIMO);
